...
conf = (SparkConf().setAppName("Total Data Count"))
...
kudu_all_data_table = "impala::default.particle_test"
kudu_counts_table = "impala::default.particle_counts_total_data"
...
kudu_events_count = kudu_events_df.rdd.map(lambda x: x.data).flatMap(lambda x: x.split(' ')).map(lambda x: (x, 1)).reduceByKey(add)
